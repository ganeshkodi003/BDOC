...
public class sendingmail_appointment {
    ...
    public String sendingmail(String from, String host, String to,  String cc, String username, String password, String ref_no) {
        ...
        try {
            ...
            message.setSubject("APPOINTMENT LETTER");
         ...
         /*   List<String> f = y;
            String[] parts = f.get(0).split("\n");
            String u = parts[0];
            String j = parts[1];
           // String k = parts[2];
          //  System.out.println("fghjkl"+k);
           ...
           // hh.add(k);*/
            ...
            for (CandEvalFormEntity atts : places) {
                try {
                    byte[] imageData = atts.getAppointment(); // Retrieve image data from the database
                    if (imageData != null) {
                        // Assuming attachmentName variable holds the name of the attachment
                    	   String attachmentName = "Bornfire-AppointmentLetter-" + ref_no + ".pdf";
            ...
            for (CandEvalFormEntity atts : placess) {
                try {
                    byte[] imageData = atts.getSalarystru(); // Retrieve image data from the database
                    if (imageData != null) {
                        // Assuming attachmentName variable holds the name of the attachment
                    	   String attachmentName = "Bornfire-SalaryStructure-" + ref_no + ".pdf";
                        MimeBodyPart attachmentPart = new MimeBodyPart();
                        attachmentPart.setDataHandler(new DataHandler(new ByteArrayDataSource(imageData, "image/pdf"))); // Assuming JPEG format
                        attachmentPart.setFileName(attachmentName); // Set the attachment file name
                        multipart.addBodyPart(attachmentPart);
            ...
            // Add attachments
           /* for (String fileName : hh) {
                try {
                   // String filePath = "C:\\newbhrps\\bhrps\\BHRPS_NEW\\downloadreports\\";
                	// String filePath = env.getProperty("output.exportpath");// Replace with the actual file path
                //	String filePath="D:\\";
