     ...
     try
     {
        ...
        // System.out.println(workingDays+""+weekend);
     	...
     	for(int i=0;i<dates.size();i++){
     	    ...
     	    sql.setLength(0);
     	    sql.append("Select count(RECORD_DATE) as holicount from HOLIDAY_MASTER where RECORD_DATE='"+df.formatdate(ds)+"' and DEL_FLG='N' ");
...
public int countHoliDays(int year, int month) {
	 /*  System.out.println("YEARRRRRR"+year);
	   System.out.println("MONTHHHHH"+month);*/
		Calendar calendar = Calendar.getInstance();
	    // Note that month is 0-based in calendar, bizarrely.
	    calendar.set(year, month - 1, 1);
	    String monthalp ="";
	...
	System.out.println("DAYSinMONTH==="+daysInMonth);
	   ...
	   String monthname=df.formatmonth(Integer.toString(month));
		...
		sql.append("Select count(HOLIDAY_DESC) as holicount from HOLIDAY_MASTER where cal_year='"+year+"' and cal_month='"+monthname+"' and DEL_FLG='N'");
	System.out.println("sql QUERY"+sql.toString());
		try {
			ps = conn.prepareStatement(sql.toString());
			ResultSet rs = ps.executeQuery();
